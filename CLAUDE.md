...
**⚠️ 중요: 프로그램 업데이트 작업 시 반드시 `docs/update_plan.md` 파일을 참고하세요.**

업데이트 작업 전에 다음을 확인하세요:
1. `docs/update_plan.md` 파일을 읽고 현재 진행 중인 업데이트 계획 확인
2. 작업 내용을 업데이트 계획에 기록
3. 작업 완료 후 진행 상황 업데이트
4. GitHub push 후 서버 테스트는 사용자가 직접 수행

## 중요 사항

- **모든 답변은 한국어로 작성**: Claude Code는 이 프로젝트에서 항상 한국어로 응답해야 함
- **예시 코드 제공 불필요**: 설명만 제공하고 코드 예시는 생략
- **개발 및 테스트 워크플로**:
  - 코드 개발은 개발 컴퓨터에서만 진행
  - 개발 컴퓨터는 서버에 연결되어 있지 않아 서버 테스트 불가능
  - 배포 프로세스: GitHub push → 서버 컴퓨터에서 git pull → 직접 테스트 진행
  - 서버 테스트는 사용자가 직접 수행
