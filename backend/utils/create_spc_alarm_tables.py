...
spc_alarms, measurement_change_history 테이블을 신규 생성합니다.
기존 테이블에는 영향 없음 (create_all은 이미 존재하는 테이블을 건너뜀).

사용법:
    python backend/utils/create_spc_alarm_tables.py
"""
import sys
import os
...
def create_spc_alarm_tables():
    """spc_alarms, measurement_change_history 테이블 생성"""
    db = SessionLocal()

    try:
        logger.info("SPC 알람 테이블 생성을 시작합니다...")
        ...
        # 생성된 테이블 확인
        for table_name in ['spc_alarms', 'measurement_change_history']:
            verify_query = text(f"""
...
if __name__ == "__main__":
    logger.info("=" * 60)
    logger.info("SPC 알람 테이블 생성")
    logger.info("=" * 60)

    try:
        create_spc_alarm_tables()
