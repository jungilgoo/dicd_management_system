...
### 2026-06-02 - SPC 분석 기간 드롭다운 수정
- **목적**: SPC 분석 페이지에서 최근 6개월/최근 1년 선택 시 실제 분석 기간이 30일로 처리되는 문제 수정
- **위치**: 공통 기간 파라미터 유틸리티 (`frontend/js/utils.js`)
- **작업 내용**:
  - [x] `prepareApiDateParams()`에 `180`, `365` 매핑 추가
  - [x] PHOTO/ETCH SPC 페이지의 기존 드롭다운 값(`180`, `365`)과 API 파라미터 매핑 일치 확인
- **테스트**:
  - [x] 정적 확인: 최근 6개월은 `days=180`, 최근 1년은 `days=365`로 반환
  - [ ] 서버 배포 후 SPC 분석 화면에서 사용자 직접 테스트
